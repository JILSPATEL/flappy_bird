    ...
    while True:
        for event in pygame.event.get():
            ...
            # If the user presses space or up key, start the game for them
                ...
                text_y1 = vertical + game_images['player'].get_height() + 10  # Adjust the 10 value as needed
...
def mainGame():
    ...
    while True:
        ...
        if not crashed:
            crashTest = isGameOver(horizontal, vertical, upperPipes,
...
def displayMessage(message):
    ...
    window.blit(text, (window_width / 2 - text.get_width() / 2, window_height / 3 - text.get_height() / 2))
...
def isGameOver(horizontal, vertical, upperPipes, lowerPipes):
    ...
    for pipe in upperPipes:
        ...
        if (vertical < pipeHeight + pipe['y'] and abs(horizontal - pipe['x']) < game_images['pipe'][0].get_width()):
            return True

    for pipe in lowerPipes:
        if (vertical + game_images['player'].get_height() > pipe['y']) and abs(horizontal - pipe['x']) < \
